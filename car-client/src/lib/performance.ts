...
export const start = (label: string, startTime = process.hrtime.bigint(), options:{silent:boolean}={silent:false}) => {
    ...
    if (!options.silent) logger.debug(`Timer started with label: ${chalk.yellowBright(label)}`, "PERFORMANCE")
}
...
export const log = (label: string, time: bigint = getRawTime(label)) => {
    ...
    if (!timers[label]) throw new Error("A timer with that label doesn't exist.")
        logger.debug(`${chalk.yellowBright(label)} took ${chalk.yellowBright(formatRawTime(time))} to complete.`, "PERFORMANCE")
...
export const exists = (label: string) => timers[label] ? true : false
