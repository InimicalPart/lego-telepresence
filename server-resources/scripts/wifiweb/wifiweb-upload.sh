...
#! Clean up the "build" folder in $ROOT_DIR/wifiweb locally
echo "[WIFIWEB] Cleaning up the build folder in $ROOT_DIR/wifiweb"
rm -rf $ROOT_DIR/wifiweb/build

#! Download all dependencies for the project in $ROOT_DIR/wifiweb locally
echo "[WIFIWEB] Downloading all dependencies for the project in $ROOT_DIR/wifiweb"
sudo -u $USR bash -i -c "npm --prefix $ROOT_DIR/wifiweb i > /dev/null"

#! Build the project in $ROOT_DIR/wifiweb locally
echo "[WIFIWEB] Building the project in $ROOT_DIR/wifiweb"
sudo -u $USR bash -i -c "npm --prefix $ROOT_DIR/wifiweb run build > /dev/null"
